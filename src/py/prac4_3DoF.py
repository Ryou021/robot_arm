...
def update(a):
    ...
    #真ん中で区切る線
    canvas.create_line(320,0,320,400, fill='black', width=3)
    #ロボットアーム土台
    canvas.create_rectangle(10, 320, 90, 370, fill = 'red')
    canvas.create_rectangle(40, 245, 60, 320, fill = 'blue')
    canvas.create_rectangle(440, 320, 520, 400, fill = 'red')
    canvas.create_rectangle(470, 350, 490, 370, fill = 'blue')
...
#      joint2_y=draw_yaxis(arm_len1*math.sin(math.radians(armb_theta))+125)+arm_len2*math.sin(math.radians(-armc_theta-armb_theta))
    ...
    #実際のServo出力
    print("Servo6：",int(-armb_theta*8000/270+9365))
    print("Servo4：",int(-math.degrees(math.atan2(joint2_y-joint1_y,joint2_x-joint1_x))*8000/270+6839))
...
s2 = tkinter.Scale(root,orient='v',variable=var2,from_=211,
                   to=-125,length=160,command=update)
s2.place(x=260, y=55)
update(0)
canvas.pack()
root.mainloop()
